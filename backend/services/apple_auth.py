...
class AppleAuthService:
    ...
    async def fetch_apple_public_keys(self):
        ...
        if self.apple_public_keys is None or (self.apple_last_key_fetch + self.apple_key_cache_exp) < current_time:
            async with httpx.AsyncClient() as client:
                response = await client.get(APPLE_PUBLIC_KEY_URL)
                keys = response.json()["keys"]
                self.apple_public_keys = {key["kid"]: jwk.construct(key) for key in keys}
    ...
    def register_or_update_user(self, claims: dict) -> User:
        ...
        auth_method = self.db.query(UserAuthMethod).filter_by(user_id=user.id, provider='apple').first()
        if not auth_method:
            auth_method = UserAuthMethod(user_id=user.id, provider='apple', provider_user_id=claims['sub'])
